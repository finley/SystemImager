...
SBINARIES := si_addclients si_cpimage si_getimage si_lint si_mkdhcpserver si_mkdhcpserver_legacy si_mkdhcpstatic_legacy si_mkautoinstallscript si_mvimage si_pushupdate si_pushinstall si_rmimage si_mkrsyncd_conf si_mkclientnetboot si_netbootmond
...
ifneq (,$(wildcard /usr/*/dracut/modules.d/99base/install)) # if "" not equals second argument (not empty, thus found), we're using an old dracut)
	...
	$(SI_INSTALL) -b -m 755 $(LIB_SRC)/dracut/modules.d/$(DRACUT_MODULE_INDEX)systemimager/systemimager-xmit-oci.sh $(DRACUT_MODULES)/$(DRACUT_MODULE_INDEX)systemimager
...
install_server_libs:
	...
	$(SI_INSTALL) -m 644 $(LIB_SRC)/SystemImager/KeaConfUtils.pm  $(LIB_DEST)/SystemImager
