...
use vars qw(@EXPORT @EXPORT_OK %EXPORT_TAGS $VERSION $SIVERSION);
...
sub create_installscript {
    ...
    my $script = standard_header($vars{IMAGE});

    # first we make sure that we are not running this script
    # on a live machine
    # $script .= ensure_environment();
    
    # make a second ramdisk
    # $script .= make_stage2_mount();

    # $script .= fetch_stage2();

    # $script .= migrate_stage1();

    # $script .= pivot_root();

    # Here is the drive partitioning

    # first, stop all md devices
    # $script .= stop_raid();

    # Note: I think the disk info should be parametrized here
    # $script .= partition_disks();

    # Note: this will only get run if raid really is setup
    # $script .= start_raid();

    # $script .= format_partitions();

    # $script .= mount_partitions();

    # $script .= rsync_image();
    
    # $script .= systemconfigurator();

    # $script .= user_exit();

    # $script .= unmount_partitions();
...
#
#  standard_header generates all the standard header information that we
#  need for an autoinstall script.
#
...
#!/bin/sh
#
# SystemImager autoinstall script
# Generated by SI v$SIVERSION - InstallScript.pm v$VERSION
