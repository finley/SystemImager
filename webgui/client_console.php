...
function UpdateLogHandler(event) {
  ...
  try { 
    ...
    logText = "<tr><td>" + logInfo.TAG + "</td><td>" + logInfo.PRIORITY + "</td><td>" + logInfo.MESSAGE + "</td></tr>";
  } catch (e) {
    console.error("JSON client_log parsing error: ", e);
    // console.error("JSON: ", event.data);
    logText = "<tr><td>webgui</td><td>local0.error</td><td>" + event.data + "</td></tr>"; // BUG: invalid chars may appear and is subject to injection.
  }
...
function StatusToText(value) {
    if (value < 100) {
      ...
      if ( value <= my_statuses.length+1 ) {
        return "Status: " + my_statuses[value];
      } else {
        return "Status: Unknown(" + (100 + value) + ")";
      }
    }
}
